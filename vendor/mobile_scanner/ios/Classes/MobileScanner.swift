...
public class MobileScanner: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate, FlutterTexture {
    ...
    func start(barcodeScannerOptions: BarcodeScannerOptions?, returnImage: Bool, cameraPosition: AVCaptureDevice.Position, torch: AVCaptureDevice.TorchMode, detectionSpeed: DetectionSpeed) throws -> MobileScannerStartParameters {
        ...
        do {
            ...
            // This property is not present in older iOS SDKs (e.g. Xcode 13.2.1 / iOS 15.2).
            // Use a dynamic lookup to keep compilation working on older toolchains.
            if device.isFocusModeSupported(.autoFocus) {
                let key = "automaticallyAdjustsFaceDrivenAutoFocusEnabled"
                let setter = NSSelectorFromString("setAutomaticallyAdjustsFaceDrivenAutoFocusEnabled:")
                if device.responds(to: setter) {
                    device.setValue(false, forKey: key)
                }
            }
